...
exports.updateCandidate = async (req, res) => {
	...
	if (user_in_db) {
		if (user_in_db.username === body.username) {
			return res.status(400).json({ message: "Usuario ya exixte." });
		}
		if (user_in_db.email === body.email) {
			return res.status(400).json({ message: "Email ya exixte." });
		}
		if (user_in_db.dni === body.dni) {
			return res.status(400).json({ message: "Dni ya exixte." });
		}
	}
