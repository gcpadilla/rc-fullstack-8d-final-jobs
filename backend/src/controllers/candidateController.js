...
exports.updateCandidate = async (req, res) => {
  ...
  let user_in_db = await candidateModel.findOne({ _id: res.locals.user.id });
  ...
  try {
    ...
    const salt = await bcryptjs.genSalt(10);
    userBlanck.password = await bcryptjs.hash(body.password, salt);
    ...
    if (user_exists) {
      await candidateModel.findByIdAndUpdate(res.locals.user.id, user_in_db, {
        new: true,
      });
      return res.status(400).json({ message: "Usuario ya existe..." });
    }
